power.sim_only_one <- function(snps,
                               n.qtl,
                               p.causal,
                               prop_shared,
                               eqtl_h2,
  ...
  effect_isoform = which.max(apply(r2_mat,1,max))
  usage_vector = c(.1,.3,.5,.7,.9)
  twas_p = rep(0,length(usage_vector))
  isotwas_p = twas_p
  
  for (u in 1:length(usage_vector)){
    usage = usage_vector[u]
    usage_all = rep((1 - usage)/(n.tx-1),n.tx)
    usage_all[effect_isoform] = usage
    gene_expression = as.numeric(drop(gexpr.tot %*% usage_all))
    ...
    #### SIM GWAS
    Z_gwas = simGeno(t(snps$genotypes[]), 2000)
    colnames(Z_gwas) = colnames(Z_qtl)
    gexpr_gwas = Z_gwas %*% B_matrix
    if (gwas_var > 0){
      alpha = rnorm(1)
    }  else {alpha = 0}
    y = simTrait(gexpr_gwas[,effect_isoform] * alpha, 
                 gwas_var)
