...
traits = readxl::excel_sheets('Developmental_Associations_BeforeFOCUS_isoTWAS.xlsx') 


for (tr in traits){
  print(tr)
  
  if (file.exists(file.path('Associations_0523/Walker/',
                            tr,paste0(tr,'_isoTWAS_FOCUS.tsv')))){
    
    file.remove(file.path('Associations_0523/Walker/',
                          tr,paste0(tr,'_isoTWAS_FOCUS.tsv')))
    
  }
  
  twas = read_xlsx('Developmental_Associations_BeforeFOCUS_isoTWAS.xlsx',
                   sheet = tr)
  ...
  fwrite(new_res,
         file.path('Associations_0523/Walker/',
                   tr,paste0(tr,'_isoTWAS_FOCUS.tsv')),
         append=T,
    ...
    for (c in chr.un[1:length(chr.un)]){
      ...
      for (gr in unique(this.res.tot$Group)){
        ...
        for (i in 1:nrow(this.res)){
          aaa = readRDS(paste0('/u/scratch/a/abtbhatt/out_models_041923/Walker/isoTWAS/',
                               this.res$Gene[i],
                               '_isoTWAS.RDS'))
          aaa = subset(aaa,
        ...
        system(paste0('/u/project/pasaniuc/pasaniucdata/software/plink ',
                      ' --bfile /u/scratch/a/abtbhatt/PEC_fetal_626',
                      ' --chr ',c,
                      ' --from-bp ',min,
                      ' --to-bp ',max,
                      ' --make-bed --out temp'),
...
setwd('Associations_0523/Walker')
traits = readxl::excel_sheets('Developmental_Associations_BeforeFOCUS_TWAS.xlsx') 

for (tr in traits[11:12]){
  print(tr)
  ...
  if (length(chr.un) >= 1){
    for (c in chr.un[1:length(chr.un)]){
      ...
      for (g in unique(this.res.tot$Group)){
        ...
        snp.chr = c()
        for (i in 1:nrow(this.res)){
          aaa = readRDS(paste0('/u/scratch/a/abtbhatt/out_models_041923/Walker/TWAS/',
                               this.res$Gene[i],
                               '_TWAS.RDS'))
          aaa = subset(aaa,
                       Feature == this.res$Transcript[i])
